define(["jquery","./alasqlbase_driver"], function ($,AlaSQLBase_driver) {

    var AlaSQL_driver = function () {
        this.db = null;
        return this;
    }

    $.extend(AlaSQL_driver.prototype,AlaSQLBase_driver.prototype); // inherit from parent class

    AlaSQL_driver.prototype.buildSchema = function (args) {
        ...
        var _this = this; // preserve reference to current object through local closures
            ...
            try {
                ...
                if (window.SQL === undefined)
                {
                    ...
                    $.getScript("javascript/alasql.js", function (script, textStatus, jqXHR) {
                        window.define = window.define_tmp;
                        jsBuildSchema();
                    }).fail(function(jqxhr, settings, exception){
                        args["error"]("Your browser does not work with AlaSQL.  Try using a different browser (Chrome, Safari, Firefox, IE 9, etc...), or a newer version of your current one.");
                    });
                }
                else
                {
