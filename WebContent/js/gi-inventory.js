...
function loadOrderWithItems(soid) {
	...
	var orders = alasql("select id, status, warehouse, vendor from sorders where soid='" + soid + "'");

	gi_so_items = [];
	if(orders!= undefined && orders.length > 0 && orders[0].id !=undefined) {
		//if(orders[0].status === 2 || orders[0].status === 3 || orders[0].status === 4) {
			var selectQry = "select * from soitems where soid='" + soid + "'";
			...
			$("#goodsissue-vendor").text(getVendorById(orders[0].vendor).NAME);
